    ...
    if (role === 'Teacher') {
        return [
            ...
            // 3. Assignment
            {
                ...
                children: [
                    { label: 'sidebar_create_assignment', view: 'assignment-create-view', route: '/teacher/assignment/create' },
...
function openCreateAssignmentModal() {
    (document.getElementById('create-assignment-form') as HTMLFormElement).reset();
...
async function handleCreateAssignment() {
    ...
    const data = {
        title: (document.getElementById('asg-title') as HTMLInputElement).value,
        description: (document.getElementById('asg-desc') as HTMLInputElement).value,
        type: (document.getElementById('asg-type') as HTMLInputElement).value,
        points: parseInt((document.getElementById('asg-points') as HTMLInputElement).value),
        due_date: (document.getElementById('asg-date') as HTMLInputElement).value
    };
...
function loadGroupAssignmentsMock(groupId) {
    const list = document.getElementById('academics-assignments-list') as HTMLInputElement;
    if (groupId === 1) {
        list.innerHTML = `
...
function updateEngagementFileName() {
    const input = document.getElementById('engagement-pdf-input') as HTMLInputElement;
    const display = document.getElementById('engagement-filename') as HTMLInputElement;
    if (input.files && input.files[0]) {
...
async function analyzeEngagementDocs() {
    ...
    // Switch tabs safely
    const resultsTabBtn = document.getElementById('engagement-results-tab') as HTMLInputElement;
    if (resultsTabBtn) {
        resultsTabBtn.disabled = false;
        const tabTrigger = bootstrap.Tab.getOrCreateInstance(resultsTabBtn);
        tabTrigger.show();
    }
    ...
    try {
        ...
        const response = await fetchAPI('/ai/analyze-engagement', {
            method: 'POST',
            body: formData,
...
function renderEngagementResults(data) {
    const loader = document.getElementById('engagement-loader') as HTMLInputElement;
    const content = document.getElementById('engagement-content') as HTMLInputElement;

...
async def get_student_assignments(student_id: str):
    ...
    # 1. Standard Assignments (via Groups)
    assignments = c.execute("""
        SELECT a.id, a.title, a.due_date, a.type, g.name as course_name
        FROM assignments a
    ...
    student = c.execute("SELECT grade FROM students WHERE id = ?", (student_id,)).fetchone()
...
async def analyze_engagement_pdf(file: UploadFile = File(...)):
    ...
    logger.info(f"Engagement Analysis started for file: {file.filename}")
    try:
        ...
        try:
            ...
            # Limit to first 12 pages for better context coverage
        ...
        if len(text) < 50:
             raise HTTPException(status_code=400, detail="The PDF contains insufficient text to analyze. Ensure it's not just images.")

        # 2. Construct Prompt for AI
        system_prompt = (
            "You are an expert pedagogical consultant. Analyze the text to see if it's educational.\n"
            "Return JSON ONLY:\n"
            "{\n"
            '  "is_educational": boolean,\n'
            '  "summary": "string",\n'
            '  "activities": ["List of 3-5 ideas"],\n'
            '  "real_life_examples": ["3 examples"],\n'
            '  "discussion_questions": ["3 questions"],\n'
            '  "games": ["2 games"],\n'
            '  "message": "Reasoning if not educational"\n'
            "}"
        )

        user_prompt = f"Content:\n\n{text[:12000]}" # Slightly more context

        # 3. Call AI (Using Sync client but wrapped for safety or we can stick to sync if needed)
        # For now, we manually handle JSON parsing more robustly
        ...
        logger.info(f"AI Response received ({len(response_content)} bytes)")

        # 4. Parse Response (Robustly)
        try:
            # Handle potential markdown wrappers even with json_object mode
            ...
            logger.error(f"JSON Parse Error: {je}. Raw Content: {response_content[:200]}")
...
async def grade_submission(sub_id: int, grade: float = Body(..., embed=True), feedback: str = Body(..., embed=True)):
    conn = get_db_connection()
    c = conn.cursor()
    try:
